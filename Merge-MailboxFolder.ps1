...
function LoadLibraries()
{
    ...
    foreach ($dllName in $dllNames)
    {
        ...
        if ($searchProgramFiles)
        {
            if ($dll -eq $null)
            {
	            $dll = Get-ChildItem -Recurse "C:\Program Files (x86)" -ErrorAction Ignore | Where-Object { ($_.PSIsContainer -eq $false) -and ( $_.Name -eq $dllName ) }
	            if (!$dll)
	            {
		            $dll = Get-ChildItem -Recurse "C:\Program Files" -ErrorAction Ignore | Where-Object { ($_.PSIsContainer -eq $false) -and ( $_.Name -eq $dllName ) }
	            }
            }
        }
...
Function GetFolder()
{
    ...
    if ($FolderPath.ToLower().StartsWith("wellknownfoldername"))
    {
        ...
        if ($RootFolder -ne $null)
        {
            ...
            #{
            #    LogVerbose "[GetFolder]WellKnownFolder $wkf was found, no further path to process"
            #    return $Folder
            #}
